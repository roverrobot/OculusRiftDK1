...
static void check_default_mesh(const DK1DistortionMesh *mesh, DK1Eye eye) {
    ...
    double center_x = eye == DK1_EYE_LEFT ? -0.423898531375 : 0.423898531375;
    ...
    check_near("center.r.x", center->tan_eye_angles_r.x, 0.0, 1e-12);
    check_near("center.g.x", center->tan_eye_angles_g.x, 0.0, 1e-12);
    check_near("center.b.x", center->tan_eye_angles_b.x, 0.0, 1e-12);
...
static void test_custom_grid_mesh(const char *home) {
    ...
    if (!make_config_dir(dir_path, sizeof(dir_path), home) ||
        ...
        !write_text_file(config_path, "0 10 8 10\n")) {
